...
Releases are handled by manually bumping and commiting new semver values in the [library package](./packages/react-async-modal-hook) and running `pnpm publish`. This obviously requires appropriate permissions to publish to npm, so in practice releases will be handled by the repository maintainers.
